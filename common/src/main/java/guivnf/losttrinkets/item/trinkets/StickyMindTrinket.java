package guivnf.losttrinkets.item.trinkets;

import net.minecraft.world.entity.LivingEntity;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.phys.AABB;
import guivnf.losttrinkets.api.LostTrinketsAPI;
import guivnf.losttrinkets.api.trinket.Rarity;
import guivnf.losttrinkets.api.trinket.Trinket;
import guivnf.losttrinkets.item.Itms;

import java.util.List;

public class StickyMindTrinket extends Trinket<StickyMindTrinket> {
    public StickyMindTrinket(Rarity rarity, Properties properties) {
        super(rarity, properties);
    }

    public static boolean shouldPreventTeleport(LivingEntity entity) {
        AABB bb = new AABB(entity.blockPosition()).inflate(16);
        List<Player> players = entity.level().getEntitiesOfClass(Player.class, bb);
        for (Player player : players) {
            if (LostTrinketsAPI.getTrinkets(player).isActive(Itms.STICKY_MIND.get())) {
                return true;
            }
        }
        return false;
    }
}
