...
public class WitherHandTrinket extends Trinket<WitherHandTrinket> {
    public WitherHandTrinket(Rarity rarity, Properties properties) {
    ...
    public static float onHurt(LivingEntity entityLiving, DamageSource source, float amount) {
        ...
        if (entity instanceof Player player) {
            ...
            if (trinkets.isActive(Itms.WITHER_HAND.get())) {
                entityLiving.addEffect(new MobEffectInstance(MobEffects.WITHER, 120, 0));
            }
        }
        return amount;
    }
